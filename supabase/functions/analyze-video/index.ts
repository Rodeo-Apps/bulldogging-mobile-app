...
// analyze-video edge function for steer wrestling.
//
...
const ANALYSIS_SCHEMA = {
  ...
  properties: {
    ...
    phases: {
      type: "object",
      additionalProperties: false,
      required: ["catch", "throw", "footwork", "hazing"],
      properties: {
          catch: { type: "string" },
          throw: { type: "string" },
          footwork: { type: "string" },
          hazing: { type: "string" },
      },
    },
...
const SYSTEM_PROMPT =
  "You are an expert steer wrestling coach. Analyze the athlete's run from the supplied frames. " +
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: SYSTEM_PROMPT + " JSON schema: " + JSON.stringify(ANALYSIS_SCHEMA) },
          {
            role: "user",
            content: [
              { type: "text", text: "Analyze this steer wrestling run." },
              ...images,
            ],
          },
        ],
    ...
    if (supabaseUrl && serviceKey && user_id) {
      ...
      await admin.from("video_analysis_results").insert({
        user_id,
        event_type: body.event_type ?? "bulldogging",
        video_url,
