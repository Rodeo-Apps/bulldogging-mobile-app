-- 003 platform layer + bulldogging_runs for bulldogging-mobile-app
...
-- Discipline run log for steer wrestling.
create table if not exists public.bulldogging_runs (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  time_seconds numeric,
  barrier_broken boolean not null default false,
  hazer_name text,
  hazer_effectiveness text,
  four_legs_same_direction boolean not null default false,
  steer_returned_to_standing boolean not null default false,
  notes text,
  created_at timestamptz not null default now()
);

create index if not exists bulldogging_runs_user_idx on public.bulldogging_runs (user_id, created_at desc);

alter table public.bulldogging_runs enable row level security;

drop policy if exists "bulldogging_runs_owner_all" on public.bulldogging_runs;
create policy "bulldogging_runs_owner_all" on public.bulldogging_runs
